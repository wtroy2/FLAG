...
functionalAnnot=`grep -P "${suffix}\t" simplifiedFunctional.tsv`
echo "${functionalAnnot}"
if [ "${program}" == "entap" ]; then
    EggNOGSeedOrtholog=`echo "$functionalAnnot" | awk -F '\t' '{ print $2 }'`
    EggNOGPredictedGene=`echo "$functionalAnnot" | awk -F '\t' '{ print $3 }'`
    EggNOGTaxScope=`echo "$functionalAnnot" | awk -F '\t' '{ print $4 }'`
    EggNOGDescription=`echo "$functionalAnnot" | awk -F '\t' '{ print $5 }'`
    if [[ "$EggNOGPredictedGene" == "" ]]; then
        EggNOGPredictedGeneLine2=`echo "$functionalAnnot" | awk -F '\t' '{ print $6 }'`
    ...
    # query  seed_ortholog   eggNOG_OGs     Description     Preferred_name
    EggNOGSeedOrtholog=`echo "$functionalAnnot" | awk -F '\t' '{ print $2 }'`
    EggNOGPredictedGene=`echo "$functionalAnnot" | awk -F '\t' '{ print $5 }'`
    EggNOGTaxScope=`echo "$functionalAnnot" | awk -F '\t' '{ print $3 }'`
    EggNOGDescription=`echo "$functionalAnnot" | awk -F '\t' '{ print $4 }'`
    if [[ "$EggNOGTaxScope" != "" ]]; then
        EggNOGTaxScope=${EggNOGTaxScope##*|}
        #EggNOGPredictedGene=${EggNOGPredictedGeneSuffix%%_*}
    fi
