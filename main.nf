...
combinestructwfunct(functionalAnnotation.out.functionalAnnot.combine(CombineAndFilter.out.gtfLenient).combine(Channel.from(params.speciesScientificName)).combine(Channel.from(params.lineage)).combine(maskedGenome).combine(Channel.from(params.funcAnnotProgram)).combine(Channel.fromPath(params.flagdb)).combine(Channel.from(params.multi_percent_same_exon_match)).combine(Channel.from(params.multi_splign_coverage_min)).combine(Channel.from(params.multi_reference_splign_coverage_min)).combine(Channel.from(params.multi_pasa_coverage_min)).combine(Channel.from(params.multi_reference_pasa_coverage_min)).combine(Channel.from(params.multi_miniprot_coverage_min)).combine(Channel.from(params.multi_is_busco)).combine(Channel.from(params.multi_num_orthologs)).combine(Channel.from(params.single_percent_same_exon_match)).combine(Channel.from(params.single_splign_coverage_min)).combine(Channel.from(params.single_reference_splign_coverage_min)).combine(Channel.from(params.single_pasa_coverage_min)).combine(Channel.from(params.single_reference_pasa_coverage_min)).combine(Channel.from(params.single_miniprot_coverage_min)).combine(Channel.from(params.single_is_busco)).combine(Channel.from(params.single_num_orthologs)).combine(Channel.from(params.single_multi_ratio)),filesToFilter)
